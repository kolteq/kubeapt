...
- **Resource-targeted selection** — `scanner.WithPolicyResources([]scanner.GVR{…})` limits the scan to policies whose `matchConstraints` target the given resources, identified by group/version/resource. Matching is group-aware and needs no cluster (exact compare against the policy's plural resources, no Kind→resource guessing); the version is accepted but not used for selection, and a policy using an `apiGroups`/`resources` wildcard is always kept.
...
Select only the policies that target specific resources — for example NetworkPolicies in a particular API group:

```go
sc, _ := scanner.New(bundle, scanner.WithPolicyResources([]scanner.GVR{
    {Group: "networking.k8s.io", Resource: "networkpolicies"}, // Version left empty: any version
    {Group: "", Resource: "services"},                         // empty Group is the core group
}))
result, _ := sc.Scan(ctx, manifests)
```
